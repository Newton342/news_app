...
class StockMarketTab extends StatelessWidget {
  const StockMarketTab({super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder<HomeController>(
        id: GetBuilderId.stockMarket,
        builder: (HomeController controller) {
          return controller.isLoadingStockMarket
              ? const Center(
                  child: CircularProgressIndicator(),
                )
              : controller.stockMarketData.isEmpty
                  ? const Center(
                      child: Text("No data"),
                    )
                  : NotificationListener<ScrollNotification>(
                      onNotification: (notification) {
                        if (notification is ScrollEndNotification &&
                          ...
                          controller.stockMarketPagination();
                        }
                        return false;
                      },
                      child: ListView.separated(
                          key: const PageStorageKey(GetBuilderId.stockMarket),
                          padding: const EdgeInsets.symmetric(vertical: 16.0),
                          itemBuilder: (context, index) {
                            final data = controller.stockMarketData[index];
                          ...
                          itemCount: controller.stockMarketData.length),
                    );
        });
  }
}
